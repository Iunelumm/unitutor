  ...
  sessionFiles: router({
    ...
    upload: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (session.studentId !== ctx.user.id && session.tutorId !== ctx.user.id) {
          throw new TRPCError({ code: 'FORBIDDEN', message: 'Not authorized to upload files to this session' });
        }

        return await db.createSessionFile({
          sessionId: input.sessionId,
    ...
    list: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (session.studentId !== ctx.user.id && session.tutorId !== ctx.user.id) {
          throw new TRPCError({ code: 'FORBIDDEN', message: 'Not authorized to view files for this session' });
        }

        return await db.getSessionFiles(input.sessionId);
      }),
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const file = await db.getSessionFile(input.fileId);
        if (!file) {
          throw new TRPCError({ code: 'NOT_FOUND', message: 'File not found' });
        }
        if (file.uploaderId !== ctx.user.id) {
          throw new TRPCError({ code: 'FORBIDDEN', message: 'Not authorized to delete this file' });
        }

        return await db.deleteSessionFile(input.fileId);
      }),
  }),

  // Session Notes Router
  sessionNotes: router({
    // Create or update note for a session
    upsert: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (session.studentId !== ctx.user.id && session.tutorId !== ctx.user.id) {
          throw new TRPCError({ code: 'FORBIDDEN', message: 'Not authorized to add notes to this session' });
        }

        const authorRole = session.studentId === ctx.user.id ? 'student' : 'tutor';
        return await db.upsertSessionNote({
          sessionId: input.sessionId,
    ...
    get: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (session.studentId !== ctx.user.id && session.tutorId !== ctx.user.id) {
          throw new TRPCError({ code: 'FORBIDDEN', message: 'Not authorized to view notes for this session' });
        }

        return await db.getSessionNotes(input.sessionId);
      }),
  }),

  // Favorite Tutors Router
  favoriteTutors: router({
    ...
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        // Verify tutor exists
        const tutor = await db.getUser(input.tutorId);
        if (!tutor) {
          throw new TRPCError({ code: 'NOT_FOUND', message: 'Tutor not found' });
        }

        return await db.addFavoriteTutor({
          studentId: ctx.user.id,
          tutorId: input.tutorId,
          notes: input.notes,
        });
    ...
    remove: protectedProcedure
      .input(z.object({ tutorId: z.number() }))
      .mutation(async ({ ctx, input }) => {
        return await db.removeFavoriteTutor(ctx.user.id, input.tutorId);
      }),

    // List favorite tutors
    list: protectedProcedure
      .query(async ({ ctx }) => {
        return await db.getFavoriteTutors(ctx.user.id);
      }),
    ...
    isFavorite: protectedProcedure
      .input(z.object({ tutorId: z.number() }))
      .query(async ({ ctx, input }) => {
        const favorite = await db.isFavoriteTutor(ctx.user.id, input.tutorId);
