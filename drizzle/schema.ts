...
/**
 * Session files table - for storing files shared during sessions
 */
export const sessionFiles = mysqlTable("sessionFiles", {
  id: int("id").autoincrement().primaryKey(),
  sessionId: int("sessionId").notNull().references(() => sessions.id),
  ...
  fileType: varchar("fileType", { length: 50 }).notNull(), // pdf, ppt, doc, image, etc.
...
/**
 * Session notes table - for post-session notes by tutors and students
 */
export const sessionNotes = mysqlTable("sessionNotes", {
  id: int("id").autoincrement().primaryKey(),
  sessionId: int("sessionId").notNull().references(() => sessions.id),
  authorId: int("authorId").notNull().references(() => users.id),
  authorRole: mysqlEnum("authorRole", ["student", "tutor"]).notNull(),
...
/**
 * Favorite tutors table - for students to save their preferred tutors
 */
export const favoriteTutors = mysqlTable("favoriteTutors", {
  id: int("id").autoincrement().primaryKey(),
  studentId: int("studentId").notNull().references(() => users.id),
  tutorId: int("tutorId").notNull().references(() => users.id),
  notes: text("notes"), // student's private notes about this tutor
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

export type FavoriteTutor = typeof favoriteTutors.$inferSelect;
export type InsertFavoriteTutor = typeof favoriteTutors.$inferInsert;
